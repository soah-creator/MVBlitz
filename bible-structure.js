// Bible structure data for validation
// Format: { bookName: [versesInChapter1, versesInChapter2, ...] }
...
function getChapterCount(book) {
  return BIBLE_STRUCTURE[book] ? BIBLE_STRUCTURE[book].length : 0;
}

// Get number of verses in a specific chapter
function getVerseCount(book, chapter) {
  if (!BIBLE_STRUCTURE[book]) return 0;
...
function isValidReference(book, chapter, verse) {
  ...
  // Handle verse ranges like "5-6"
  if (typeof verse === 'string' && verse.includes('-')) {
    const [start, end] = verse.split('-').map(v => parseInt(v.trim()));
    return start >= 1 && end <= maxVerse && start <= end;
  }

  const verseNum = parseInt(verse);
  return verseNum >= 1 && verseNum <= maxVerse;
}
